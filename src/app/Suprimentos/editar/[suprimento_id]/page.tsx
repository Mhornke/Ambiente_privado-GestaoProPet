...
export default function AlteracaoSuprimento() {
  const { register, handleSubmit, reset } = useForm<Inputs>();
  const params = useParams() as { suprimento_id: string }
  
  async function alteracaoSuprimento(data: Inputs) {
    ...
    console.log("id pelo params", params.suprimento_id);


    const response = await fetch(`${process.env.NEXT_PUBLIC_URL_API}/suprimentos/${params.suprimento_id}`,
      {
        method: 'PUT',
        headers: {
    ...
    if (response.status == 200) {
      toast.success("Suprimento Alterado com sucesso!");
      reset();
    } else {
      toast.error("Houve um erro ao Alterado o suprimento.");
    }
  ...
  return (
    <div className="flex flex-col gap-4 p-7">
      ...
      <form action="" className="flex flex-col gap-4" onSubmit={handleSubmit(alteracaoSuprimento)}>
